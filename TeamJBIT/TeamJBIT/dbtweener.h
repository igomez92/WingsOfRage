...
public:
	...
	// representation of a single value 'to be tweened'. this is either a float pointer or a void function that takes a single float parameter
	...
	struct SValue
	{
		...
		SValue(float startValue, std::function<void (float)> funcValue, float fTarget)
		{
			m_funcValue = funcValue;
			isDirect = false;
			indirectValue = startValue;
			m_fpValue = &indirectValue;
			m_fTarget = fTarget;

			m_fStart = 0.0f;
		}

		//if Direct, the tween directly modifies m_fpValue,
		//if not, m_funcValue is called with the current value
		bool isDirect;
		float *m_fpValue;
		float indirectValue;
		std::function<void (float)> m_funcValue;
		float m_fTarget;
		float m_fStart;
	};
	...
	public:
		...
		CTween(CEquation *pEquation, EEasing eEasing, float fDuration, float startValue, std::function<void (float)> funcValue, float fTarget);
		~CTween();
		...
		void addValue(float startValue, std::function<void (float)> funcValue, float fTarget);
	...
	private:
		void setValue(SValue *pValue, float newValue);
	...
	void addTween(CEquation *pEquation, EEasing eEasing, float fDuration, float startValue, std::function<void (float)> funcValue, float fTarget);
