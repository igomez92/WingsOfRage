...
CDBTweener::CTween::CTween(CEquation *pEquation, EEasing eEasing, float fDuration, float startValue, std::function<void (float)> funcValue, float fTarget)
{
	m_fElapsedSec = 0.0f;
	m_fDurationSec = fDuration;
	m_pEquation = pEquation;
	m_eEasing = eEasing;
	m_pUserData = NULL;
	addValue(startValue, funcValue, fTarget);
}
...
void CDBTweener::CTween::addValue(float startValue, std::function<void (float)> funcValue, float fTarget)
{
	m_vValues.push_back(new SValue(startValue, funcValue, fTarget));
}
...
void CDBTweener::CTween::step(float fDeltaTimeSec)
{
	...
	for (TValuePts::iterator i = m_vValues.begin(); i != m_vValues.end(); i++)
	{
		...
		//if this is an indirect tween, this method has set the SValue's "indirectValue"
		//time to propagate through the desired function
		if (!pValue->isDirect)
			pValue->m_funcValue(pValue->indirectValue);
	}
...
void CDBTweener::addTween(CEquation *pEquation, EEasing eEasing, float fDuration, float startValue, std::function<void (float)> funcValue, float fTarget)
{
	CTween *pTween = new CTween(pEquation, eEasing, fDuration, startValue, funcValue, fTarget);
	pTween->addListener(&m_oTweenRelay);
	m_sTweens.insert(pTween);
}
