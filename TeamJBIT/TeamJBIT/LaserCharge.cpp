...
LaserCharge::LaserCharge(sf::Vector2f pos, sf::Vector2f dir, float timeLim, int dam)
	:dam(dam), totalTime(timeLim), pos(pos), dir(dir)
{
	sf::Texture* image = _getTexture("media/charge.png");
...
LaserCharge::~LaserCharge(void)
{

}

void LaserCharge::update(float deltaTime)
{
	accumTime += deltaTime;
...
void LaserCharge::draw(sf::RenderWindow& window)
{
	window.draw(sprite);
}

bool LaserCharge::collidesWith(Enemy& enemy)
{
