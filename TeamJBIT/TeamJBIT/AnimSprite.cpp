...
AnimSprite::AnimSprite() : timeSinceLastFrame(0), currentFrame(0) {
	//act like a regular sprite for now
	this->addAnim("default");
	this->playAnim("default");
}

void AnimSprite::setFrameSize(int width, int height) {
	frameSize.x = width;
	frameSize.y = height;
	showFrame(0);
}
...
void AnimSprite::setBorderSize(int width, int height) {
	borderSize.x = width;
	borderSize.y = height;
	showFrame(0);
}
...
bool AnimSprite::playAnim(std::string name) {
